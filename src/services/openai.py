...
class OpenAIProvider:
    ...
    def _initialize_client(self):
        ...
        self.base_url = self.config_manager.get("OpenAIAPI", "base_url", "https://api.openai.com/v1").rstrip("/")
    ...
    def _encode_image_to_base64(self, pil_image: Image.Image) -> str:
        buffered = BytesIO()
        # Convert to RGB if RGBA to avoid issues with some formats/APIs if needed, 
        # but PNG supports RGBA. OpenAI usually handles it.
    ...
    def process_image(
        ...
        target_language = self.config_manager.get("OpenAIAPI", "target_language", "Chinese")
        source_language = self.config_manager.get("OpenAIAPI", "source_language", fallback="Japanese").strip() or "Japanese"
        
        # Reuse glossary logic
...
"""
        ...
        # We reuse the prompt template from Gemini as it's designed for the same JSON output format
        ...
        try:
            if progress_callback:
                progress_callback(25, f"发送请求给 OpenAI Compatible API ({self.model_name})...")

            timeout = int(self.config_manager.get("OpenAIAPI", "request_timeout", "60"))
            
            # Handle proxy if enabled globally
            proxies = {}
    ...
    def _parse_json_response(self, raw_text: str) -> Optional[List[Dict[str, Any]]]:
        # Reuse similar parsing logic as Gemini, but OpenAI usually returns cleaner JSON if json_object mode is on
        ...
        try:
            ...
            # OpenAI might wrap it in a key if we asked for it, but our prompt asks for a list.
            # However, response_format={"type": "json_object"} requires the output to be an object, not a list.
            # So we might need to adjust the prompt or handle the object wrapper.
            # Wait, if the prompt asks for a list, and we force json_object, OpenAI might complain or wrap it.
            # Let's assume for now we handle both list and dict wrapper.
            ...
            for item_idx, item in enumerate(items):
                ...
                # Normalize BBox (same logic as Gemini)
